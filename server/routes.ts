...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/profit-breakdown/:period/:marketplace', requireAuth, requireRole(['partner']), async (req, res) => {
    try {
      ...
      const totalRevenue = analytics.reduce((sum: number, data: any) => sum + parseFloat(data.revenue || '0'), 0);
      const totalOrders = analytics.reduce((sum: number, data: any) => sum + (data.orders || 0), 0);
